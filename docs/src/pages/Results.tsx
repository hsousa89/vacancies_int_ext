...
export function Results() {
  ...
  // Local Search States for the checkboxes
  ...
  const qzpMunicipalityMap = useMemo(() => {
    ...
    flatResults.forEach(v => {
      ...
      // Only add clean municipality names (strip out extra parenthesis if they exist)
  ...
  const displayResults = useMemo(() => {
    return baseResults.filter(v => {
      ...
      return matchZone && matchConcelho && matchSchool;
    });
  }, [baseResults, selectedZones, selectedConcelhos, selectedSchools]);
    ...
    <div className="flex flex-col gap-4">
        ...
        <details className="bg-white rounded-xl border border-slate-200 shadow-sm mb-2 group">
          <summary className="p-4 font-semibold text-slate-800 cursor-pointer flex justify-between items-center list-none">
            <span className="flex items-center gap-2">
              ...
              Filtros Avançados {(selectedZones.length + selectedConcelhos.length + selectedSchools.length) > 0 && 
                <span className="bg-rose-600 text-white text-xs px-2 py-0.5 rounded-full">
                  {selectedZones.length + selectedConcelhos.length + selectedSchools.length}
                </span>
              }
            </span>
          ...
          <div className="p-4 border-t border-slate-100 flex flex-col gap-6">
            ...
            {scope === 'school' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Municipalities List with Search */}
                <div className="flex flex-col gap-2">
                  ...
                  <input 
                    ...
                    value={concelhoQuery}
                    onChange={(e) => setConcelhoQuery(e.target.value)}
            ...
            {(selectedZones.length > 0 || selectedConcelhos.length > 0 || selectedSchools.length > 0) && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-20">
        {displayResults.map((vacancy) => {
          // Compile the full list of municipalities for this specific QZP
          ...
          return (
            <div key={vacancy.id} className="p-4 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col hover:shadow-md transition-shadow">
              ...
              <div className="flex justify-between items-start gap-2 mb-3">
                {/* Updated QZP Badge with Sub-Municipalities */}
                <div 
                  className="flex-1 flex flex-col bg-blue-50 px-2.5 py-1.5 rounded-md min-w-0" 
                  title={`${vacancy.qzp}: ${municipalitiesList}`}
                >
                ...
                {/* Updated Count Badge (Green = +, Red = -, Slate = 0) */}
